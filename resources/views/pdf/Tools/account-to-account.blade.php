...
<head>
    <meta charset="UTF-8">
    <title>انتقال بین حسابات  - {{ $conversion->type }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="document">
        <div class="header">
            <h1>صرافی {{ Auth::guard('tools')->user()->company_name ?? 'صرافی' }}</h1>
        ...
        <div class="contact-info">
            <div style="margin-bottom: 5px;">
                <strong>تماس:</strong> {{ Auth::guard('tools')->user()->phone ? '+93' . Auth::guard('tools')->user()->phone : 'نامشخص' }}
            </div>
            <div>
                <strong>آدرس:</strong> {{ Auth::guard('tools')->user()->address ? 'افغانستان - ' . Auth::guard('tools')->user()->address : 'نامشخص' }}
            </div>
        </div>
