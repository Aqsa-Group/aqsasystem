    ...
    <div class="w-full max-w-[1400px] mx-auto p-4">
        ...
        <div class="mb-8 text-right space-y-4">
            ...
            <p class="text-base text-gray-600 dark:text-gray-300 vazir">
                @if($isMonthlyView)
                نمایش و گزارش حضور و غیاب ماه {{ $persianMonths[$monthFilter] }} سال {{ $yearFilter }}
                @else
        ...
        <div class="bg-[#F5F5F5] dark:bg-blue-900/20 rounded-xl mt-5 p-5 mb-6"
            ...
            <div class="flex items-center justify-between">
                <div class="text-2xl font-bold text-blue-600 dark:text-blue-400 vazir">
                    گزارش ماه {{ $persianMonths[$monthFilter] }} سال {{ $yearFilter }}
                </div>
                <div class="text-lg text-gray-700 dark:text-gray-300 vazir">
                    {{ $this->getJalaliDaysInMonth($yearFilter, $monthFilter) }} روز کاری
                </div>
            </div>
        </div>
        @endif
        ...
        @if($selectedStaff && !$isMonthlyView)
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 mb-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white vazir">
                        ثبت حضور و غیاب برای {{ $selectedStaff->name }}
                    </h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        معاش روزانه: {{ number_format($selectedStaff->final_salary / 30) }} افغانی
                    </p>
                </div>
                <div class="text-sm text-gray-600 dark:text-gray-300">
                    معاش صبح: {{ number_format($selectedStaff->final_salary / 60) }} افغانی
                    <span class="mx-2">|</span>
                    معاش شام: {{ number_format($selectedStaff->final_salary / 60) }} افغانی
                </div>
            </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" wire:model="attendanceData.morning_present"
                                    class="sr-only peer" {{ $currentAttendance['leave_type']==='morning' ||
                                ...
                                <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                                    {{ $attendanceData['morning_present'] ? 'حاضر' : 'غایب' }}
                                </span>
                            </label>
                        </div>

                        <!-- Time Input -->
                        @if($attendanceData['morning_present'])
                        <div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" wire:model="attendanceData.evening_present"
                                    class="sr-only peer" {{ $currentAttendance['leave_type']==='evening' ||
                                    $currentAttendance['leave_type']==='full_day' ? 'disabled' : '' }}>
                                <div class="w-14 h-7 bg-gray-200 peer-focus:outline-none rounded-full peer 
                                        dark:bg-gray-600 peer-checked:after:translate-x-full 
                                        peer-checked:after:border-white after:content-[''] 
                                ...
                                <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                                    {{ $attendanceData['evening_present'] ? 'حاضر' : 'غایب' }}
                                </span>
                            </label>
                        </div>

                        <!-- Time Input -->
                        @if($attendanceData['evening_present'])
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                زمان حضور شام
                            </label>
            ...
            <div class="mt-6 bg-gray-50 dark:bg-gray-700 rounded-xl p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach(['none' => 'بدون مرخصی', 'morning' => 'مرخصی صبح', 'evening' => 'مرخصی شام', 'full_day'
                    =>
                    'مرخصی کامل'] as $value => $label)
                    <label
                        class="flex items-center p-3 border rounded-lg cursor-pointer 
                             {{ $attendanceData['leave_type'] === $value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                        <input type="radio" wire:model="attendanceData.leave_type" value="{{ $value }}"
                            class="w-4 h-4 text-blue-600 focus:ring-blue-500">
                        <span class="mr-3 text-sm text-gray-700 dark:text-gray-300">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>

                @if($attendanceData['leave_type'] !== 'none')
                <div class="mt-4">
...
<div class="bg-[#F5F5F5] dark:bg-gray-800 rounded-2xl shadow-lg p-6 mb-6"
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold text-gray-900 dark:text-white vazir">
            حضور و غیاب روز {{ $persianDate }}
        </h2>
        <div class="text-lg font-bold text-blue-600 dark:text-blue-400">
            مجموع معاش امروز: <span class="text-red-500">{{ number_format(
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse($staffsArray as $index => $staff)
                @php
                $attendance = \App\Models\Sarafi\StaffAttendance::where('staff_id', $staff['id'])
                ->whereDate('attendance_date', $selectedDate)
                ->first();
                ...
                <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col items-center gap-2">
                            <button wire:click="quickAttendance({{ $staff['id'] }}, 'morning_present')"
                                class="p-2 rounded-lg {{ $attendance && $attendance->morning_present ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' }}">
                                @if($attendance && $attendance->morning_present)
                            ...
                            @if($attendance && $attendance->morning_time)
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col items-center gap-2">
                            <button wire:click="quickAttendance({{ $staff['id'] }}, 'evening_present')"
                                class="p-2 rounded-lg {{ $attendance && $attendance->evening_present ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' }}">
                                @if($attendance && $attendance->evening_present)
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                                        d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                        clip-rule="evenodd" />
                            ...
                            @if($attendance && $attendance->evening_time)
                            <span class="text-xs text-gray-600 dark:text-gray-400">
                    ...
                    <!-- Full Day Leave -->
                    <td class="px-6 py-4 text-center">
                        <button wire:click="quickAttendance({{ $staff['id'] }}, 'full_day_leave')"
                            class="p-2 rounded-lg {{ $attendance && $attendance->leave_type === 'full_day' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' }}">
                            @if($attendance && $attendance->leave_type === 'full_day')
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                    ...
                    <td class="px-6 py-4">
                        ...
                        {{ $attendance->morning_present && $attendance->evening_present ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 
                          ...
                          'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300') }}">
                            @if($attendance->leave_type === 'full_day')
                                مرخصی کامل
                            @elseif($attendance->leave_type === 'morning')
                                مرخصی صبح
                            @elseif($attendance->leave_type === 'evening')
                                مرخصی شام
                            @elseif($attendance->morning_present && $attendance->evening_present)
                                حاضر کامل
                            @elseif($attendance->morning_present)
                                حاضر صبح
                            @elseif($attendance->evening_present)
                                حاضر شام
                            @else
                                غایب
                            @endif
                    ...
                    <td class="px-6 py-4 font-medium">
                        @if($attendance)
                        {{ number_format($attendance->daily_salary) }} افغانی
                        @else
                        0 افغانی
                        @endif
                    </td>
...
@endif
        ...
        <div class="bg-[#F5F5F5] dark:bg-gray-800 rounded-2xl shadow-lg p-6"
            ...
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white vazir">
                    @if($isMonthlyView)
                    گزارش تفصیلی ماه {{ $persianMonths[$monthFilter] }} سال {{ $yearFilter }}
                    @else
                    گزارش ماه {{ $persianMonths[$monthFilter] }} سال {{ $yearFilter }}
                    @endif
                </h2>
                <div class="text-lg font-bold text-blue-600 dark:text-blue-400">
                    مجموع معاش این ماه کل کارمندان: <span class="text-red-500">{{ number_format($totalMonthlySalary)
                        }}</span> افغانی
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @forelse($monthlyReport as $index => $report)
                        @php
                        ...
                        <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4">{{ $index + 1 }}</td>
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ $report['staff']->name }} {{ $report['staff']->fathername }}
                            </td>
                            <td class="px-6 py-4">{{ $report['staff']->job }}</td>
                            <td class="px-6 py-4">{{ $report['full_days'] }} روز</td>
                            <td class="px-6 py-4">{{ $report['half_days'] }} روز</td>
                            <td class="px-6 py-4">{{ $report['leave_days'] }} روز</td>
                            <td class="px-6 py-4">{{ $report['absent_days'] }} روز</td>
                            <td class="px-6 py-4 font-bold text-green-600 dark:text-green-400">
                                {{ number_format($report['total_salary']) }} افغانی
                            </td>
                            <td class="px-6 py-4">
