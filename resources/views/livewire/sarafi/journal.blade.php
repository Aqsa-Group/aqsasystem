            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-7 gap-4 mb-5 items-end">
                ...
                <div>
                    ...
                    <button wire:click="printReport" wire:loading.attr="disabled" wire:target="printReport" class="w-full h-[60px] flex items-center justify-center gap-2
                        ...
                        <span wire:loading wire:target="printReport">در حال ایجاد گزارش...</span>
                    </button>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr class="text-black border-b  dark:text-white border-[#D9D9D9]
                               ...
                               transition-colors">
                                ...
                                <td class="px-4 py-4 font-bold">{{ $item->currency_fa }}</td>
                                <td class="px-4 py-4 text-green-600">{{ number_format($item->receipt_cash, 2) }}
                                </td>
                                <td class="px-4 py-4 text-red-600">{{ number_format($item->withdrawal_cash, 2) }}
                                </td>
                                <td class="px-4 py-4 text-green-600">{{ number_format($item->receipt_bank, 2) }}
                                </td>
                                <td class="px-4 py-4 text-red-600">{{ number_format($item->withdrawal_bank, 2) }}
                                </td>
                                <td class="px-4 py-4 font-bold {{ $item->balance_cash >= 0 ? 'text-green-600' : 'text-red-600' }}"
                                    dir="ltr">
                                    {{ number_format($item->balance_cash, 2) }}
                                </td>
                                <td class="px-4 py-4 font-bold {{ $item->balance_bank >= 0 ? 'text-green-600' : 'text-red-600' }}"
                                    dir="ltr">
    ...
    <script>
        function customerSearch() {
            return {
                isOpen: false,
                searchQuery: '',
                selectedCustomerId: @entangle('selectedCustomer').live,
