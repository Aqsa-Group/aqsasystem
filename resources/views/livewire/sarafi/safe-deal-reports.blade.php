<div>
    ...
    <div class="flex flex-col px-4 space-y-3">
        <h1 class="text-[25px] vazir"> گزارشات تبادله بین صندوق ها</h1>
        <h1 class="text-[#8C8C8C]  pb-6">مجموعه مبالغ تبدیل شده هر ارز صندوق ها</h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <!-- ================= جدول طلب نقدی ================= -->
        <div class="bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">

            <div class="p-3 font-bold text-center border-b dark:border-gray-700 vazir">
                وضعیت تبادله های نقدی
            </div>

            <div class="overflow-x-auto">
                <table class="min-w-[700px] w-full text-sm text-gray-700 dark:text-gray-300">

                    <!-- Header: ارزها -->
                    <thead class="bg-gray-100 dark:bg-gray-800">
                        <tr>
                            @foreach($currencies as $currency)
                            <th class="px-4 py-3 text-center border-b whitespace-nowrap vazir">
                                {{ $currency['name_fa'] }}
                            </th>
                            @endforeach
                        </tr>
                    ...
                    <tbody>
                        <tr class="bg-white dark:bg-gray-900">
                            @foreach($currencies as $currency)
                            @php
                            $amount = $cashBalances[$currency['code']] ?? 0;
                            @endphp

                            <td class="px-4 py-4 text-center">
                                <div dir="ltr" class="flex flex-col items-center gap-1 font-number
                ...
                <table class="min-w-[700px] w-full text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <tbody>
                        <tr class="bg-white dark:bg-gray-900">
                            @foreach($currencies as $currency)
                            @php
                            $amount = $bankBalances[$currency['code']] ?? 0;
                            @endphp
        ...
        <div class="flex flex-col
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <p class="flex items-center gap-2 text-[18px] inter whitespace-nowrap">
                    ...
                    {{ $transactionId ? 'فورم ویرایش برداشت' : 'فورم برداشت' }}
                </p>
            ...
            <form wire:submit.prevent="submitTransaction" class="dark:text-white">
                ...
                <div class="mt-2  grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 w-full gap-3">
                    ...
                    <div class="flex-1 w-full">
                        <div class="relative w-full">
                            <label class="block text-[16px] font-medium dark:text-white text-black mb-1 vazir"> انتقال
                                به حساب (اختیاری)
                            </label>
                            <div x-data="{
                                    searchValue: '',
                ...
                <div
                    ...
                    <button type="button" wire:click="submitAndPrint" wire:loading.attr='disabled'
                        ...
                        <span wire:loading wire:target="submitAndPrint" class="flex items-center justify-center gap-2">
                            <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                    stroke-width="4"></circle>
