<div>
        ...
        <div
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['today']['AFN']) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-rose-700">دالر:</span>
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['today']['USD']) }}</span>
                </div>
        ...
        <div
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['week']['AFN']) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-green-700">دالر:</span>
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['week']['USD']) }}</span>
                </div>
        ...
        <div
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['month']['AFN']) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-blue-700">دالر:</span>
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['month']['USD']) }}</span>
                </div>
        ...
        <div
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['total']['AFN']) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-purple-700">دالر:</span>
                    <span class="text-lg font-bold">{{ number_format($withdrawalStats['total']['USD']) }}</span>
                </div>
    ...
    <div class="flex flex-col lg:flex-row gap-5 mt-4 ">
        ...
        {{-- فرم برداشت --}}
        <div class="flex flex-col bg-[#F5F5F5] w-full lg:w-[474px] p-[10px] h-[600px] rounded-[12px] space-y-2"
            ...
            <div class="flex flex-row justify-between p-[16px] border border-[#8C8C8C] rounded-[12px] flex-wrap mb-5">
                <p class="flex justify-center items-center text-center">
                    ...
                    {{ $withdrawalId ? 'فورم ویرایش برداشت' : 'فورم ثبت برداشت از صندوق' }}
                </p>
            </div>
            ...
            <form wire:submit.prevent="submitWithdrawal" class="space-y-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="relative w-full">
                            ...
                            <div class="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500">
                                @if($currency === 'AFN') افغانی
                                @elseif($currency === 'USD') دالر
                                @else تومان
                                @endif
                            </div>
                        </div>
                ...
                <!-- دکمه‌ها -->
                <div
                    class="grid grid-cols-1 md:grid-cols-2  gap-1 py-4 justify-center items-center text-center flex-wrap">
                    <button type="submit"
                        class="bg-[#61B138] text-[18px] vazir font-semibold rounded-[8px] px-16 py-3 text-white">
                        {{ $withdrawalId ? 'بروزرسانی' : 'ثبت برداشت' }}
                    </button>

                    <button type="button" wire:click="resetForm"
                        class="bg-[#DD2424] text-[18px] vazir font-semibold rounded-[8px] px-16 py-3 text-white">
                        {{ $withdrawalId ? 'لغو ویرایش' : 'انصراف' }}
                    </button>
                </div>
            </form>
        </div>
        ...
        {{-- جدول برداشت‌ها --}}
        <div class="flex-1 flex flex-col bg-[#F5F5F5] p-3 md:p-4 lg:p-6 rounded-[12px]  mt-[30px] sm:overflow-x-auto md:mt-0 lg:mt-0 mx-auto  md:mx-auto lg:mx-auto w-full "
            style="box-shadow: 0px 4px 4px 0px #00000040, 0 0 0 0 #3B82F6;">
                        ...
                        <tbody>
                            ...
                            <tr class="text-black border-b border-[#D9D9D9] bg-transparent">
                                ...
                                <td class="px-4 py-4 vazir text-[14px] md:text-[16px] font-medium text-center w-80">
                                    {{ $typesFa[$withdrawal->type] ?? $withdrawal->type }}
                                </td>
                                ...
                                <td class="py-4 text-center w-[68]">
                                    <div class="flex justify-center gap-3">
                                        ...
                                        <button wire:click="print({{ $withdrawal->id }})"
                                            class="w-12 h-12 flex items-center justify-center rounded-full transition-colors"
                                            title="پرینت">
