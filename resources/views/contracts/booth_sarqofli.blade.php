...
<body
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin: 0 auto;">
                            <tr>
                                ...
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">
                                    {{ number_format($booth->sarqofli_price ?? 0) }}؋</td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">مناصفه</td>
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">
                                    {{ number_format(($booth->sarqofli_price ?? 0) / 2) }}؋</td>
                            </tr>
                        </table>
    ...
    <table style="width: 100%; font-size: 9pt; direction: rtl; margin-bottom: 8pt;">
     ...
     <div style="font-size: 10.7pt; line-height: 1.8; text-align: justify; margin-bottom: 12pt;  color:black;"
        ...
        در بند غرفه نمبر ({{ $booth->number ?? '_____' }}) در طبقه ({{ $booth->floor ?? '_____' }}) طرف
        ({{ $booth->side ?? '_____' }}) مارکت ({{ $booth->market->name ?? '_____' }})
        واقع همین جایداد متذکره که مساحت آن ({{ $booth->size ?? '_____' }}) متر مربع و بدین
        حدود اربعه محدود است شمالاً ({{ $booth->north ?? '____________________' }}) شرقاَ
        ({{ $booth->east ?? '____________________' }})
        جنوباً ({{ $booth->south ?? '____________________' }}) غرباً ({{ $booth->west ?? '____________________' }})
        ...
        در حالی که موصوف نیز دارای اهلیت شرعی و قانونی خویش می‌باشد به مبلغ ({{ number_format($booth->sarqofli_price ?? 0) }}؋)
        افغانی به حرف ({{ $booth->sarqofli_fa_price ?? '___' }}) که مناصفه آن ({{ number_format(($booth->sarqofli_price ?? 0) / 2) }}؋) به
        سر قفلی داده‌ام.
    </div>
