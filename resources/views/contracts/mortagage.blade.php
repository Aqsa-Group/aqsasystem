...
<body
    ...
    @php
        ...
        $startDate = $shop->contract_start ? Jalalian::fromDateTime($shop->contract_start) : null;
        $endDate = $shop->contract_end ? Jalalian::fromDateTime($shop->contract_end) : null;
    ...
    <div style="font-size: 10.7pt; line-height: 1.8; text-align: justify; margin-bottom: 12pt; color:black;">
        ...
        در حالی که دارای اهلیت کامل شرعی و قانونی خویش می‌باشم، دوکان واقع در طبقه ({{ $shop->floor ?? '---' }})
        نمبر ({{ $shop->number ?? '---' }}) را
        برای محترم ({{ $customer->fullname ?? '---' }}) ولد ({{ $customer->father_name ?? '---' }}) ولدیت
        ({{ $customer->grand_father ?? '---' }}) دارنده تذکره ({{ $customer->id_number ?? '---' }})،
        از تاریخ ({{ $startDate ? $startDate->format('Y/m/d') : '__ / __ / ۱۴__' }}) الی
        ({{ $endDate ? $endDate->format('Y/m/d') : '__ / __ / ۱۴__' }}) به مدت
        ({{ $shop->contract_duration }}) به مبلغ ({{ number_format($shop->rent_price ?? 0) }}؋)
        معادل فارسی ({{ $shop->rent_fa_price ?? '---' }}) که مناصفه آن ({{ number_format(($shop->rent_price ?? 0) / 2) }}؋)
