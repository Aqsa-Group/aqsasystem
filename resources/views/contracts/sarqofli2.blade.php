...
<head>
    <meta charset="UTF-8" />
    <title>سند سرقفلی عادلیار</title>
</head>
...
<body
    ...
    <div
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    ...
                    <td
                        ...
                        <img src="{{ public_path('storage/' . $customer->market->market_owner) }}" alt="امضاء"
                    ...
                    <td style="width: 105mm; text-align: center; padding: 0 5px;">
                        <div style="margin-bottom: 3pt; font-size: 11pt; font-weight: bold; text-align: center;">
                            سند سرقفلی خط دوکان های فردوس پلازا
                        </div>
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin: 0 auto;">
                            <tr>
                                ...
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">
                                    {{ number_format($shop->sarqofli_price ?? 0) }}؋</td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">مناصفه</td>
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">
                                    {{ number_format(($shop->sarqofli_price ?? 0) / 2) }}؋</td>
                            </tr>
                        </table>
        ...
        <td style="width: 28mm; height: 28mm; border: 1px solid #1e3a8a; text-align: center; vertical-align: middle;">
            <img src="{{ public_path('storage/' . $customer->profile_image) }}" alt="امضاء"
                style="width: 28mm; height: 28mm;" />
        </td>
    ...
    <table style="width: 100%; font-size: 9pt; direction: rtl; margin-bottom: 8pt;">
        <tr>
            ...
            <td style="text-align: right; vertical-align: top;">
                تاریخ:
                {{ \Morilog\Jalali\Jalalian::fromDateTime($customer->contract_start)->format('Y/m/d') ?? '__/__/14__' }}
            </td>
    ...
    <div style="font-size: 10.7pt; line-height: 1.8; text-align: justify; margin-bottom: 12pt;  color:black;"
        ...
        در بند دوکان نمبر ({{ $shop->number ?? '_____' }}) در طبقه ({{ $shop->floor ?? '_____' }}) طرف
        ({{ $shop->side ?? '_____' }}) مارکت ({{ $shop->market->name ?? '_____' }})
        واقع همین جایداد متذکره که مساحت آن ({{ $shop->size ?? '_____' }}) متر مربع و بدین
        حدود اربعه محدود است شمالاً ({{ $shop->north ?? '____________________' }}) شرقاَ
        ({{ $shop->east ?? '____________________' }})
        جنوباً ({{ $shop->south ?? '____________________' }}) غرباً ({{ $shop->west ?? '____________________' }})
        بالای محترم ({{ $customer->fullname ?? '_____' }})
        ولد ({{ $customer->father_name ?? '_____' }}) ولدیت ({{ $customer->grand_father ?? '_____' }}) مسکونه
        ({{ $customer->address ?? '_____' }}) دارنده نمبر تذکره ({{ $customer->id_number ?? '_____' }})
        در حالی که موصوف نیز دارای اهلیت شرعی و قانونی خویش می‌باشد به مبلغ ({{ number_format($shop->sarqofli_price ?? 0) }}؋)
        افغانی به حرف ({{ $shop->sarqofli_fa_price ?? '___' }}) که مناصفه آن ({{ number_format(($shop->sarqofli_price ?? 0) / 2) }}؋) به
        سر قفلی داده‌ام.
    </div>
    ...
    <div style="font-size: 10pt; margin-bottom: 18pt; color:black;">
        ...
        <div style="margin-top: 6pt; padding-right: 16pt; line-height: 1.3;">
            ...
            <p style="margin-bottom: 2pt;">
                ۲- هرگاه مشتری بخواهد ملکیت هذا را به سرقفلی کرایه و رهن بدهد طبق نرخ روز
                حق اولویت از مالک آن می باشد. در صورت عدم نیاز مالک. مشتری آن را به
                شخص دیگری به سرقفلی داده می تواند در این صورت طرفین بیع جدید مکلف اند هر
                کدام بایع و مشتری یک فیصد از اصل قیمت را به مالک پرداخت نمایند.

            </p>
            <p style="margin-bottom: 2pt;">۳- پرداخت فیصدی مسئولیت دوکان مذکور به دوش شخص سرقفلی کننده می باشد.</p>
            <p style="margin-bottom: 2pt;">۴- جمع آوری کرایه ملکیت سرقفلی داده شده طبق لایحه تعیین شده به دوش مالک می
                باشد.</p>
            <p style="margin-bottom: 2pt;">۵- بدون استحضاری مهر و امضاء مالک. رهن و سرقفلی ملکیت هذا هیچ اعتباری ندارد.
            </p>
            <p style="margin-bottom: 2pt;">۶- در هرسه سال طبق مقررات کرایه مالکیت افزوده می شود.</p>
            <p style="margin-bottom: 2pt;">۷- بر خورد مناسب با مشتری و عابرین و یاری در نظم مارکت.</p>
        </div>
