...
<body
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    ...
                    <td
                        ...
                        <img src="{{ public_path('storage/' . $shopkeeper->market->market_owner) }}" alt="امضاء"
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin: 0 auto;">
                            <tr>
                                ...
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">
                                    {{ number_format($shop->price ?? 0) }}؋</td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">مناصفه</td>
                                <td style="border: 1px solid #1e3a8a; padding: 3px;">
                                    {{ number_format(($shop->price ?? 0) / 2) }}؋</td>
                            </tr>
                        </table>
    ...
    <div style="font-size: 10.7pt; line-height: 1.8; text-align: justify; margin-bottom: 12pt;  color:black;"
        ...
        بالای محترم ({{ $shopkeeper->fullname ?? '_____' }})
        ولد ({{ $shopkeeper->father_name ?? '_____' }}) ولدیت ({{ $shopkeeper->grand_father ?? '_____' }}) مسکونه
        ({{ $shopkeeper->address ?? '_____' }}) دارنده نمبر تذکره ({{ $shopkeeper->national_id ?? '_____' }})
        در حالی که موصوف نیز دارای اهلیت شرعی و قانونی خویش می‌باشد به مبلغ ({{ number_format($shop->price ?? 0) }}؋)
        افغانی به حرف ({{ $shop->fa_price ?? '___' }}) که مناصفه آن ({{ number_format(($shop->price ?? 0) / 2) }}؋) به
        سر قفلی داده‌ام.
    </div>
