...
<div class="w-full max-w-[1465px] bg-white shadow-sm backdrop-blur-2xl border border-[#D7E5EC] rounded-[12px] mt-10 p-6 mx-auto">
        ...
        <table class="w-full text-sm md:text-base text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody class="text-[16px] md:text-[18px] text-gray-800 bg-white">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(count($balances) > 0): ?>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $balances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $balance): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
    ...
    if (this.selectedDate) {
        ...
        if (fieldName === 'startDate') {
            window.Livewire.find('<?php echo e($_instance->getId()); ?>').setStartDate(formattedDate);
        } 
        // برای endDate
        else if (fieldName === 'endDate') {
            window.Livewire.find('<?php echo e($_instance->getId()); ?>').setEndDate(formattedDate);
        }
        this.closePicker();
    } else {
        this.setToday();
