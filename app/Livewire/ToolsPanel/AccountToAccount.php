...
use App\Models\Tools\SendToAccount;
...
class AccountToAccount extends Component
{
    ...
    public function render()
    {
        ...
        if (!$user) {
            return view('livewire.tools-panel.account-to-account', [
                'customers' => collect(),
    ...
    public function calculateAmounts()
    {
        ...
        if ($this->withdrawal_amount && $this->commission_amount) {
            ...
            if ($withdrawal >= $commission) {
                ...
                $this->transferable_amount = '0';
                $this->received_amount = '0';
                $this->commission_amount = $this->withdrawal_amount;
            }
        } else {
 ...
 private function updateCurrenciesDefault($balances, $totalInUsd)
{
    ...
    {
        $validationRules = [
            'withdrawalAccount' => 'required|integer|exists:tools.customers,id',
        ...
        if ($this->transactionType === 'باتفاوت') {
            $validationRules['commission_amount'] = 'required|numeric|min:0|max:' . $this->withdrawal_amount;
            $validationRules['commissionAccount'] = 'required|integer|exists:tools.customers,id';
        }
    ...
    private function generateConversionPdf($conversionId)
    {
        try {
            ...
            $html = view('pdf.Tools.account-to-account', compact('conversion'))->render();
            $mpdf->WriteHTML($html);

            $fileName = 'انتقال بین حسابات' . $conversion->id . '_' . $conversion->type . '.pdf';
